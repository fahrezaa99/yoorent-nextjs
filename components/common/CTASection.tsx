...
export default function CTASection() {
  ...
  // Tombol 1: Mulai Sewa Sekarang
  const handleMulaiSewa = () => {
    router.push("/cari-barang");
  };

  // Tombol 2: Daftar Sebagai Pemilik
  const handleDaftarPemilik = () => {
    if (!isLoggedIn) {
      setOpenLogin(true);
    } else {
  ...
  return (
    <>
      <section className="py-16 px-4 bg-gradient-to-r from-indigo-400 via-blue-500 to-purple-500">
        <div className="max-w-2xl mx-auto text-center rounded-2xl shadow-2xl p-10 bg-transparent">
          <h2 className="text-3xl md:text-4xl font-extrabold text-white mb-4">
            Siap Mulai Sewa di YooRent?
          </h2>
          <p className="text-lg text-blue-50 mb-10">
            Bergabunglah dengan ribuan pengguna yang telah merasakan kemudahan YooRent
          </p>
          <div className="flex flex-col md:flex-row items-center justify-center gap-4 mt-4">
            <button
              className="px-8 py-3 rounded-lg font-semibold text-white bg-white bg-opacity-90 text-purple-700 hover:bg-opacity-100 shadow transition text-base md:text-lg"
              onClick={handleMulaiSewa}
              type="button"
            >
              Mulai Sewa Sekarang
            </button>
            <button
              className="px-8 py-3 rounded-lg font-semibold text-white border border-white hover:bg-white hover:text-purple-700 transition text-base md:text-lg"
              onClick={handleDaftarPemilik}
              type="button"
            >
